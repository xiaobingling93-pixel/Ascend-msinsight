...
test.describe('Source', () => {
        ...
        test('test_source_loadDataSuccess', async ({sourcePage}) => {
                ...
                await expect(mainContent).toHaveScreenshot('loadDataSuccess.png');
        });
        // 筛选条件变化，修改Core选项
        // 预期：
        ...
        test('test_source_coreSelectChange', async ({page, sourcePage}) => {
                ...
                await expect(mainContent).toHaveScreenshot('coreSelectChange.png');
        });
        // 筛选条件变化，修改Source选项
        // 预期：代码表的源代码Source变化
        test('test_source_sourceSelectChange', async ({page, sourcePage}) => {
                ...
                await coreSelect.open();
                await coreSelect.selectOption('/home/huangning/code/samples/operator/AddCustomSample/KernelLaunch/AddKernelInvocationNeo/build/auto_gen/ascendc_kernels_sim/auto_gen_add_custom.cpp');
                const codeTable = sourceFrame.locator('#CodeTable');
                await expect(codeTable).toContainText('#ifndef __ADD_CUSTOM__KERNEL_FUN_H_');
        });
        // 点击代码行，联动指令表
        ...
        test('test_source_CodeTable_interact_InstructionTable', async ({page, sourcePage}) => {
                ...
                await expect(mainContent).toHaveScreenshot('CodeTable_interact_InstructionTable.png');
        });
        // 点击指令表，联动代码行
        // 点击右侧指令表（不是每个都有关联代码行）
        // 预期：
        ...
        test('test_source_InstructionTable_interact_CodeTable', async ({page, sourcePage}) => {
                ...
                await sourcePage.mouseOut();
                await expect(mainContent).toHaveScreenshot('InstructionTable_interact_CodeTable.png');
        });
        // 切换语言，查看中文界面
        test('test_source_Chinese_Language', async ({page, sourcePage}) => {
                ...
                await sourcePage.mouseOut();
                await expect(mainContent).toHaveScreenshot('Chinese_Language.png');
        });
});