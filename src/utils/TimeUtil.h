...
public:
    ...
    const Time Now() const
    {
        ...
        constexpr int SINCE_YEAR = 1900;
        constexpr int SINCE_MONTH = 1;
        constexpr int MS_DURATION = 1000;
...
#endif
        Time time = {
            .year = nowTime.tm_year + SINCE_YEAR,
            .month = nowTime.tm_mon + SINCE_MONTH,
            .day = nowTime.tm_mday,
            .hour = nowTime.tm_hour,
            .minute = nowTime.tm_min,
            .second = nowTime.tm_sec,
            .millisecond = static_cast<int>(durationInMs.count() % MS_DURATION)
        };
        return time;
    }
    ...
    const int64_t MsToNs(int64_t time) const
    {
        return time * MS_TO_NS;
    }

    const int64_t NsToMs(int64_t time) const
    {
        return int64_t (time / MS_TO_NS);
    }
...
private:
    ...
    const int MS_TO_NS = 1000000; // ms -> ns
};
