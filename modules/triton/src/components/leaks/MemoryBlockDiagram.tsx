...
export const MemoryBlockDiagram = ({ session }: { session: Session }): JSX.Element => {
    ...
    const handleClick = (ev: MouseEvent): void => {
        ...
        if (isClick.current) {
            ...
            const { sizeInfo, renderOptions: { transform, zoom } } = session.leaksWorkerInfo;
            const { minTimestamp, maxTimestamp } = sizeInfo;
            const { width } = session.leaksWorkerInfo.renderOptions.viewport;
            ...
            const clickTimestamp = visibleMinSize + visibleRange * ratio;

            runInAction(() => {
                session.markLineInfo.clickTimestamp = clickTimestamp;
            });

            workerClickItem({ clientX: clickX, clientY: rect.height - (ev.clientY - rect.top) });
        }
    };
