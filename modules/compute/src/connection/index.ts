...
abstract class BaseConnector {
    ...
    constructor(getTargetWindow: GetTragetWindows) {
        ...
        Object.defineProperty(window, 'onmessage', {
            configurable: false,
            set: () => {
                log(this.printErrMsg('the property \'window.onmessage\' in used, reassign it is invalid'));
            },
        });
    }

    send<T extends EventHanlder>(body: SendParams<T>, reject?: (reason?: any) => void): void {
        if (this.invalidFunc) {
            this.invalidFunc();
            return;
        }
        this._targetWindows = this._getTargetWindows();
        const checkRes = (body.to !== undefined && window.parent !== null && window.parent[body.to]?.postMessage === undefined) ||
            this._targetWindows.length === 0;
        if (checkRes) {
...
class ClientConnector extends BaseConnector {
    private readonly _msgSequence: Map<FetchSequenceID, (x: unknown) => void> = new Map();
