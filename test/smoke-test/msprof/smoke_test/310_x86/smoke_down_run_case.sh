...
function Main()
{
    ...
    ./auto_ssh.sh "cd /home/msprof_smoke_test/smoke_test/310_x86/testcase; bash run_all.sh ${command_type} ${MSPROF_VERSION}" ${host_ip} ${host_user} ${host_pwd} | tee -a ${run_log}
    CopyLogPROFData
    sshpass -p ${host_pwd} scp -r -q -o StrictHostKeyChecking=no ${host_user}@${host_ip}:/home/msprof_smoke_test/smoke_test/310_x86/testcase/result.txt ./
