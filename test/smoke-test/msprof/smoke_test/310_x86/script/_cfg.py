...
class ConfigPaths:
    ...
    oneStepPytorch_path = ""
    loopTensorFlow_path = ""
    noLoopTensorFlow_path = ""
    netMindSpore_path = ""
    resNet50MindSpore_path = ""
    singleOpMorePytorch_path = ""
    singleOpPytorch_path = ""
    stepPytorch_path = ""
    singleOpTensorFlow_path = ""
    yolov5MindSpore_path = ""
    fpBpTensorFlow_path = ""
    op_cost_baseline_path = "/home/msprof_smoke_test/smoke_test/310_x86/baseline/op_meantime_baseline_310x86.json"
...
class ConfigValues:
    ...
    host = "host"
    device = "device"
    aclapi_switch = ["ACL_PROF_ACL_API=on", "ACL_PROF_TASK_TIME=on", "ACL_PROF_AICPU=on", "ACL_PROF_L2CACHE=off",
                     "ACL_PROF_RUNTIME_API=on", "ACL_PROF_SYS_HARDWARE_MEM_FREQ=on", "ACL_PROF_LLC_MODE=capacity",
    ...
    acljson_switch = ["aicpu=on", "l2=off", "hccl=off", "task_time=on", "ascendcl=on", "runtime_api=on",
                      "sys_hardware_mem_freq=on", "llc_profiling=bandwidth", "sys_io_sampling_freq=on",
    ...
    pyaclapi_switch = ["ACL_PROF_ACL_API=on", "ACL_PROF_TASK_TIME=on", "ACL_PROF_AICPU=on", "ACL_PROF_L2CACHE=off",
    ...
    tensorflow_switch = ["hccl=off", "aicpu=on", "l2=on", "msproftx=off", "task_time=on", "runtime_api=on",
                         "sys_hardware_mem_freq=on", "llc_profiling=capacity", "sys_io_sampling_freq=on",
                         "sys_interconnection_freq=on", "dvpp_freq=on", "host_sys=cpu,mem", "host_sys_usage=cpu,mem",
                         "host_sys_usage_freq=on"]
    ...
    msprofbin_switch = ["ascendcl=on", "model-execution=on", "runtime-api=on", "task-time=on", "aicpu=on",
                        ...
                        "sys-io-profiling=on", "dvpp-profiling=on", "llc-profiling=capacity", "aic-mode=task-based",
...
class ConfigCmd:
    ...
    # common cmd
    all_switch = "{}; {}; " \
                 "{} --ascendcl=on --model-execution=on --runtime-api=on --task-time=on --ai-core=on " \
                 ...
                 "--sys-pid-sampling-freq=10 --sys-io-profiling=on --sys-io-sampling-freq=100 --dvpp-profiling=on " \
    ...
    single_sys_switch = "{}; {}; " \
                        ...
                        "--dvpp-profiling=off".format(ConfigPaths.log_level, ConfigPaths.toolkit_env_path,
                                                      ConfigPaths.msprofbin_path, ConfigValues.dev_id)
    single_app_switch = "{}; {}; " \
                        "{} --ascendcl=off --model-execution=off --runtime-api=off --task-time=off --ai-core=off " \
                        "--aicpu=off --dvpp-profiling=off {}".format(ConfigPaths.log_level,
                                                                     ConfigPaths.toolkit_env_path,
                                                                     ConfigPaths.msprofbin_path, app_switch)
...
class ConfigTableHeaders:
    table_header = {
        ...
        "ai_core_utilization": {
            "sample-based": {
                ...
                "MemoryUB": "Core ID,ub_read_bw_vector(GB/s),ub_write_bw_vector(GB/s),ub_read_bw_scalar(GB/s),"
                            "ub_write_bw_scalar(GB/s),ub_read_bw_mte(GB/s),ub_write_bw_mte(GB/s)",
        ...
        "op_statistic": {"WholeNetwork": "Model Name,OP Type,Core Type,Count,Total Time(us),Min Time(us),Avg Time(us),"
                                         "Max Time(us),Ratio(%)",
                         "SingleOp": "OP Type,Core Type,Count,Total Time(us),Min Time(us),Avg Time(us),Max Time(us),"
                                     "Ratio(%)"},
        "op_summary": {
            "sample-based": {
                "ArithmeticUtilization": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,"
                                         ...
                                         "Output Formats",
                "L2Cache": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                           ...
                           "Context ID",
                "Memory": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                ...
                "MemoryL0": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                            "Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,HF32 Eligible,Input Shapes,"
                            "Input Data Types,Input Formats,Output Shapes,Output Data Types,Output Formats",
                "MemoryUB": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                            "Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,HF32 Eligible,Input Shapes,"
                            "Input Data Types,Input Formats,Output Shapes,Output Data Types,Output Formats",
                "PipeUtilization": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                                   "Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,HF32 Eligible,Input Shapes,"
                                   "Input Data Types,Input Formats,Output Shapes,Output Data Types,Output Formats",
                "ResourceConflictRatio": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                                         "Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,"
                                         "HF32 Eligible,Input Shapes,Input Data Types,Input Formats,Output Shapes,Output Data Types,"
                                         "Output Formats",
            },
            "task-based": {
                "ArithmeticUtilization": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type"
                                         ",Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,"
                                         "HF32 Eligible,Input Shapes,Input Data Types,Input Formats,Output Shapes,Output Data Types,"
                ...
                "Memory": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                          ...
                          "ub_read_bw(GB/s),ub_write_bw(GB/s),l1_read_bw(GB/s),l1_write_bw(GB/s),l2_read_bw(GB/s),"
                          "l2_write_bw(GB/s),main_mem_read_bw(GB/s),main_mem_write_bw(GB/s)",
                "MemoryL0": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                            "Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,HF32 Eligible,Input Shapes,"
                            "Input Data Types,Input Formats,Output Shapes,Output Data Types,Output Formats,"
                ...
                "MemoryUB": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                            "Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,HF32 Eligible,Input Shapes,"
                            "Input Data Types,Input Formats,Output Shapes,Output Data Types,Output Formats,"
                            "aicore_time(us),total_cycles,ub_read_bw_mte(GB/s),ub_write_bw_mte(GB/s),"
                            "ub_read_bw_vector(GB/s),ub_write_bw_vector(GB/s),ub_read_bw_scalar(GB/s),"
                            "ub_write_bw_scalar(GB/s)",
                "PipeUtilization": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                                   "Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,HF32 Eligible,Input Shapes,"
                                   "Input Data Types,Input Formats,Output Shapes,Output Data Types,Output Formats,"
                ...
                "PipeUtilization_": "Model ID,Task ID,Stream ID,Op Name,OP Type,OP State,Task Type,Task Start Time(us),"
                                    "Task Duration(us),Task Wait Time(us),Block Dim,HF32 Eligible,Input Shapes,Input Data Types,"
                                    "Input Formats,Output Shapes,Output Data Types,Output Formats,aicore_time(us),"
                                    "total_cycles,vec_time(us),vec_ratio,mac_time(us),mac_ratio,scalar_time(us),"
                                    "scalar_ratio,mte1_time(us),mte1_ratio,mte2_time(us),mte2_ratio,mte3_time(us),"
                                    "mte3_ratio,icache_miss_rate,memory_bound,cube_utilization(%)",
                "ResourceConflictRatio": "Model Name,Model ID,Task ID,Stream ID,Infer ID,Op Name,OP Type,OP State,Task Type,"
                                         "Task Start Time(us),Task Duration(us),Task Wait Time(us),Block Dim,"
                                         "HF32 Eligible,Input Shapes,Input Data Types,Input Formats,Output Shapes,Output Data Types,"
        ...
        "task_time": "Time(%),Time(us),Count,Avg(us),Min(us),Max(us),Waiting(us),Running(us),Pending(us),Type,API,"
                     "Task ID,Op Name,Stream ID",
...
class ConfigSwitchRes:
    ...
    switch_option = {
        ...
        "sys-hardware-mem": {
            ...
            "summary": ["ddr", "hbm", "npu_mem", "prof_rule"],
        },
