...
class ConfigPaths:
    ...
    python_env = "python_version=\"python3.7.5\"; " \
                 "export LD_LIBRARY_PATH=/usr/local/${python_version}/lib:$LD_LIBRARY_PATH; " \
                 "export PATH=/usr/local/${python_version}/bin:$PATH; " \
                 "export PYTHONPATH=/usr/local/${python_version}/lib/python3.7/site-packages:$PYTHONPATH"
    ...
    toolkit_env_path = "{}; {}; source {}/set_env.sh".format(python_env, tfplugin_env_path, ascend_toolkit_path)
    ...
    nanoPipeUtilization_path = os.path.join(model_path, "Analysis_NANO")
...
class ConfigTableHeaders:
    table_header = {
        ...
        "hccl_statistic": "OP Type,Count,Total Time(us),Min Time(us),Avg Time(us),Max Time(us),Ratio(%)",
    }
