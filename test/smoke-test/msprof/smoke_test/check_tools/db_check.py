...
class DBManager:
    ...
    def check_item_in_table(cls: any, db_path: str, table_name: str, col: str, item: any):
        ...
        try:
            ...
            assert False, f"Failed to fetch data, ERROR: {_err}"
        finally:
            DBManager.destroy_db_connect(conn, curs)
        assert len(data) != 0, "Item is not in table."
            ...
            for table_name, field_content_dict in table_field_content.items():
                for field_name, required_content in field_content_dict.items():
                    ...
                    assert not missing_content, (
                        f"Field '{field_name}' in table '{table_name}' in database '{db_path}' is missing required content: {missing_content}")
        except sqlite3.Error as _err:
            assert False, f"Failed to fetch data, ERROR: {_err}"
        finally:
            DBManager.destroy_db_connect(conn, curs)
    ...
    def check_table_field_values(cls, db_path: str, table_name: str, field_names: list, comparison_func) -> None:
        ...
        try:
            ...
            assert not invalid_values, (
                f"Fields '{', '.join(field_names)}' in table '{table_name}' in database '{db_path}' contain values that do not meet the condition: {invalid_values}")
        except sqlite3.Error as _err:
            assert False, f"Failed to fetch data, ERROR: {_err}"
        finally:
            DBManager.destroy_db_connect(conn, curs)
