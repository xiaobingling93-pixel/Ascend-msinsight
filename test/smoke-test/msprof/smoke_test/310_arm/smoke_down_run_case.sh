...
function RebootDev()
{
    ...
    do
        ...
        if [ 0 -ne $? ];then
            echo "login root is Fail" | tee -a ${run_log}
            let ctime++
            continue
        else
...
function DownloadCannPkg()
{
	...
	if [ ${cann_version} == "" ];then
	    echo "Failed to get cann version" | tee ${run_log}
	fi
...
function Main()
{
    ...
    ./auto_ssh.sh "cd /home/msprof_smoke_test/smoke_test/310_arm/testcase; bash run_all.sh ${command_type} ${MSPROF_VERSION}" ${host_ip} ${host_user} ${host_pwd} | tee -a ${run_log}
    CopyLogPROFData
    sshpass -p ${host_pwd} scp -r -q -o StrictHostKeyChecking=no ${host_user}@${host_ip}:/home/msprof_smoke_test/smoke_test/310_arm/testcase/result.txt ./ | tee -a ${run_log}
...
function RunSmokeScript()
{
    ...
    ./auto_ssh.sh "cd /home/msprof_smoke_test/smoke_test/310_arm/testcase; bash run_all.sh ${command_type} ${MSPROF_VERSION}" ${host_ip} ${host_user} ${host_pwd} | tee -a ${run_log}
    sshpass -p ${host_pwd} scp -r -q -o StrictHostKeyChecking=no ${host_user}@${host_ip}:/home/msprof_smoke_test/smoke_test/310_arm/testcase/result.txt ./
