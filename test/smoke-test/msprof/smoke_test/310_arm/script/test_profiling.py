...
class TestProfiling(unittest.TestCase):
    ...
    def get_switch_res(self, switch_cmd):
        ...
        self.__logger.info("switch result: {}".format(switch_res_dict))
    ...
    def check_flashattention_valid(self):
        ...
        if self.op_summary_context.empty:
            return
        self.__logger.info(
    ...
    def check_op_statisic_avgtime(self):
        ...
        for op_type, avg_time in zip(self.op_statistic_context['OP Type'],
            ...
            if usecase_id == "test_IsSwitchMsproftx" and op_type == "SoftmaxV2":
                if not (avg_time > 15 and avg_time < 35):
                    self.__logger.error("device_{}: {} in op_statistic*.csv is not valid!".format(self.dev_id, op_type))
                    self.res += 1
                continue
            elif usecase_id == "test_IsSwitchMsproftx" and op_type == "Cast":
                if not (avg_time > 15 and avg_time < 30):
                    self.__logger.error("device_{}: {} in op_statistic*.csv is not valid!".format(self.dev_id, op_type))
                    self.res += 1
                continue
            ...
            if difference >= threshold:
                self.__logger.warning("device_{}: {} in op_statistic*.csv is not valid!".format(self.dev_id, op_type))
                return
    ...
    def check_overall_csv_valid(self):
        ...
        if "nano_pipeutilization" != self.__scene:
            self.check_aclnn_api(csv_statistic['op_summary_num'])
    ...
    def check_max_duration(self, op_type_cost):
        ...
        for op_type in op_type_cost:
            ...
            if not op_cost < value * 2.5:
                self.res += 1
                self.__logger.error(
                    f"task duration does not match baseline(2.5x), {op_type}\n{self.op_type_baseline[self.__id][op_type]}\n{op_cost}")
    ...
    def check_aclnn_api(self, op_summary_num):
        banned = ['Tiling', 'WorkspaceSize']
        for index, item in self.api_statistic_context.iterrows():
            api = str(item['API Name'])
            count = int(item['Count'])
            if not api.startswith('aclnn') or any(part in api for part in banned):
                continue
            for op_type in op_summary_num:
                if op_type not in api:
                    continue
                if op_summary_num[op_type] != count:
                    self.__logger.error(f"op type {op_type} in op_summary mismatch {api} in api_statistic")
                    self.res += 1
            ...
            if "ai_core_utilization" == csv_name or "op_summary" == csv_name:
                ...
                if not re.findall(r"aic-metrics\S+", str(self.switch_values)):
                    ...
                    if "AddOp" == self.__scene or "MsprofbinPyTorch" == self.__scene \
                        ...
                        except_res = self.cfg_header.table_header[csv_name][aic_mode[0]][
                            aic_metrics[0] + "_"]
                ...
                if "AddOp" == self.__scene or "SingleOp" == self.__scene or "MsprofbinPyTorch" == self.__scene \
                        ...
                        or "CannProfilingPyTorch" == self.__scene or "DynamicLaunch" == self.__scene \
                        or "DynamicAttach" == self.__scene or "MsprofbinMindSpore" == self.__scene:
                    except_res = self.cfg_header.table_header[csv_name]["SingleOp"]
                else:
    ...
    def view_res_file(self, output, switch_res):
        if "AclApiSubscription" == self.__scene:
            ...
            dev_dir_list, log_dir_list, output_dir_list = self.get_dev_log_output_list(prof_lst)
            try:
