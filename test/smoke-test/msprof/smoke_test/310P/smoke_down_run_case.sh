...
function DownloadCannPkg()
{
    ...
    do
        ...
        ./auto_ssh.sh "artget pull \"CANN ${cann_version}\" -ru software -rp run/Ascend-cann-kernels-310p_* -ap ${pkg_path} -user ${user} -pwd ${pwd} -o CANN" ${host_ip} ${host_user} ${host_pwd} | tee ${run_log}
...
function UpgradePersonalRepo()
{
    local personal_user=$(echo ${personal_repo} | awk -F "/" '{print $4}')
    local personal_branch=$(echo ${personal_repo} | awk -F "git-" '{print $2}')
    ./auto_ssh.sh "cd /home/fuzz_samples/msprof; git remote add ${personal_user} ${personal_repo}; git pull ${personal_user} ${personal_branch}" ${host_ip} ${host_user} ${host_pwd}
}
...
function Main()
{
    ...
    ./auto_ssh.sh "cd /home/msprof_smoke_test/smoke_test/310P/testcase; bash run_all.sh ${command_type} ${MSPROF_VERSION}" ${host_ip} ${host_user} ${host_pwd} | tee -a ${run_log}
    CopyLogPROFData
    sshpass -p ${host_pwd} scp -r -q -o StrictHostKeyChecking=no ${host_user}@${host_ip}:/home/msprof_smoke_test/smoke_test/310P/testcase/result.txt ./
...
function UpdateSmokeScriptBySsh()
{
    ...
    ./auto_ssh.sh "cd /home/msprof_smoke_test; rm -rf smoke_test; git clone -b smoke_test_master ${smokeScriptUrl}" ${host_ip} ${host_user} ${host_pwd} | tee -a ${run_log}
...
function RunSmokeScript()
{
    ...
    # 判断最终的运行结果，没有failed就返回1，所以只要不是1就失败
    grep "fail" result.txt
    if [ $? -ne 1 ];then
        exit 1
    fi
