...
from torchair_model_train import train

import glob
import tempfile

def test_profiler_torchair_with_all_params():
    epochs = 5
...
def _check_text_files(prof_dir):
    ...
    FileChecker.check_csv_items(op_statistic_path[0], {"OP Type": ["Mul", "Add"]}, fuzzy_match=False)
    ...
    FileChecker.check_csv_items(kernel_details_path[0], {"Name": ["*MatMul*", "*Add*"]})
    # 7. l2_cache.csv
    ...
    FileChecker.check_csv_items(l2_cache_path[0], {"Op Name": ["*MatMul*", "*Add*"]})
    # 8. npu_module_mem.csv
...
def _check_db_files(prof_dir: str):
    ...
    expect_tables = ["ACC_PMU", "AICORE_FREQ", "CANN_API", "COMPUTE_TASK_INFO", "CONNECTION_IDS", "CPU_USAGE",
                     ...
                     "NETDEV_STATS", "NIC", "NPU_INFO", "NPU_MEM", "NPU_MODULE_MEM", "OP_MEMORY", "PCIE",
...
if __name__ == '__main__':
    test_profiler_torchair_with_all_params()
