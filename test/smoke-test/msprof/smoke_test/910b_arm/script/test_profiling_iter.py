...
class TestProfiling(unittest.TestCase):
    ...
    def get_switch_res(self, switch_cmd):
        ...
        if soc == "910":
            ...
            if "GraphApi" == self.__scene:
                dev_json_lst = [_data for _data in dev_json_lst if _data != "fusion_op"]
                dev_csv_lst = [_data for _data in dev_csv_lst if _data != "fusion_op"]
    ...
    def view_dev_host_timeline_file(self, prof_dir):
        ...
        return dev_lst, host_lst, timeline_lst, profiler_output_lst
    ...
    def view_summary_content(self, summary_dir, summary_file):
        for index, csv in enumerate(sorted(os.listdir(summary_dir))):
            ...
            if "ai_core_utilization" == summary_file[index] or "op_summary" == summary_file[index]:
                ...
                if not re.findall(r"aic-metrics\S+", str(self.switch_values)):
                    if "AclApi" == self.__scene or "AclJson" == self.__scene or "ExportTensorFlow" == self.__scene \
                            or "SessionRunTensorFlow" == self.__scene:
                    ...
                    if "AddOp" == self.__scene or "MsprofbinPyTorch" == self.__scene \
                        ...
                        except_res = self.cfg_header.table_header[summary_file[index]][aic_mode[0]][
                            aic_metrics[0] + "_"]
                ...
                if 'ffts_off_hccl_l0' == self.__scene or 'ffts_on_hccl_l0' == self.__scene \
                        or 'analyze_communication' == self.__scene or 'nano_pipeutilization' == self.__scene \
                        or 'ffts_off_hccl_l1' == self.__scene or 'ffts_on_hccl_l1' == self.__scene:
                    except_res = except_res[11:]
                    except_res = except_res.replace('Infer ID,', '')
                    ...
                    except_res += ',aicore_time(us),total_cycles,vec_time(us),' \
                                  'vec_ratio,mac_time(us),mac_ratio,scalar_time(us),' \
                                  'scalar_ratio,mte1_time(us),mte1_ratio,mte2_time(us),' \
                                  'mte2_ratio,mte3_time(us),mte3_ratio,icache_miss_rate,' \
                                  'memory_bound,cube_utilization(%)'

            elif "step_trace" == summary_file[index]:
                ...
                if "AddOp" == self.__scene or "SingleOp" == self.__scene or "MsprofbinPyTorch" == self.__scene \
                        ...
                        or "CannProfilingPyTorch" == self.__scene or "DynamicLaunch" == self.__scene \
                        or "DynamicAttach" == self.__scene or "MsprofbinMindSpore" == self.__scene \
                        or 'ffts_off_hccl_l0' == self.__scene or 'ffts_off_hccl_l1' == self.__scene \
                        or 'ffts_on_hccl_l0' == self.__scene or 'ffts_on_hccl_l1' == self.__scene \
                        or 'analyze_communication' == self.__scene or 'nano_pipeutilization' == self.__scene:
                ...
                if "communication_statistic" == summary_file[index]:
                    cmd = "awk -F \',\' \'{{sum += $2}} END {{print sum}}\' {}".format(os.path.join(summary_dir, csv))
            ...
            if header != except_res:
                self.__logger.error('expect header: {}'.format(except_res))
                self.res += 1
            # 判断表头下第一行数据是否为 ?
    ...
    def view_iteration_msg(self, argv_path):
        ...
        try:
            ...
            return max_iter_format, True
        except Exception as error:
            self.__logger.warning("Failed to find model and iter info in this data, please check")
            return max_iter_format, False

    def view_timeline_summary_file(self, argv_path, switch_res, iter_format, get_iter_flag):
        self.__logger.info("start view {} ...".format(argv_path))
        argv_res = sorted(os.listdir(argv_path))
        self.__logger.info("file list: {}".format(argv_res))
        for index, item in enumerate(argv_res):
            if "prof_rule" not in item:
                if get_iter_flag and "_".join(iter_format) not in item:
    ...
    def view_dev_host_file(self, argv_path, switch_res):
        ...
        get_iter_flag = False
        if re.search(self.cfg_value.device, argv_path):
            max_iter_msg, get_iter_flag = self.view_iteration_msg(argv_path)
            ...
            self.__logger.info("Current Host result: {}".format(path_res))
            max_iter_msg = ""
        ...
        if os.path.exists(timeline_dir):
            self.view_timeline_summary_file(timeline_dir, switch_res, max_iter_msg, get_iter_flag)
        if os.path.exists(summary_dir):
            self.view_timeline_summary_file(summary_dir, switch_res, max_iter_msg, get_iter_flag)
    ...
    def view_res_file(self, output, switch_res):
        if "AclApiSubscription" == self.__scene:
            ...
            dev_res, host_res, timeline_res, output_res = self.view_dev_host_timeline_file(prof_lst)
            self.__logger.warning(dev_res)
            try:
                for dev in dev_res:
    ...
    def view_error_msg(self, argv_path, log_type):
        ...
        if self.__scene == "nano_pipeutilization":
            return
        if log_type == "plog":
    ...
    def case_train_scene(self, scene=None):
        ...
        cmd = ("cd {}; {}; bash run_train.sh {} --model-id={} --iteration-id={} > {}"
               .format(train_path, self.cfg_path.toolkit_env_path,
                       self.res_dir, self.__model_id, self.__iteration_id, self.slog_stdout))
        self.msprofbin_and_parse(cmd)
