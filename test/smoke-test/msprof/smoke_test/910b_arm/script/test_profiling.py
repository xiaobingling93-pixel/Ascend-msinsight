...
class TestProfiling(unittest.TestCase):
    ...
    def get_switch_value(self, switch_cmd):
        if "AclApi" == self.__scene:
            ...
            self.switch_values = self.cfg_value.graphapi_switch
                ...
                or "StepPyTorch" == self.__scene or "ImageNetTrain" == self.__scene:
            ...
            switch_open_list = ["on", "l0", "l1"]
            for switch in switch_lst:
                if switch.startswith("--"):
                    switch = switch.split("--")[1]
                    switch_status = switch.split("=")[1] if "=" in switch else None
                    if switch_status in switch_open_list:
                        if "export" in switch:
                            continue
    ...
    def get_switch_res(self, switch_cmd):
        ...
        if soc == "910B":
            ...
            if "ExportTensorFlow" == self.__scene or "MsprofbinTensorFlow" == self.__scene \
                ...
                dev_csv_lst = [_data for _data in dev_csv_lst if _data != "acl" if _data != "acl_statistic"
                               if _data != "aicpu" if _data != "fusion_op" if _data != "l2_cache"]
        ...
        if "aic-mode=task-based" in switch_values:
            dev_json_lst = [_data for _data in dev_json_lst if _data != "ai_core_utilization" if
                            _data != "ai_vector_core_utilization"]
            dev_csv_lst = [_data for _data in dev_csv_lst if _data != "ai_core_utilization" if
                           _data != "ai_vector_core_utilization"]
        if "AddOp" == self.__scene or "SingleOp" == self.__scene or "MsprofbinPyTorch" == self.__scene \
            ...
            dev_csv_lst = [_data for _data in dev_csv_lst if _data != "ge" if _data != "aicpu"
                           if _data != "fusion_op" if _data != "step_trace" if _data != "l2_cache"]
        ...
        if "Sys" == self.__scene and soc == "910B":
    ...
    def get_dev_log_output_list(self, prof_dir):
        ...
        return dev_lst, log_list, profiler_output_lst
    ...
    def check_hccl_api(self, op_summary_num):
        ...
        for op_type in hccl_statistic_dict.keys():
            ...
            if len(set(num_list)) != 1:
                state += 1
        if state:
            self.__logger.error("The number of communication operators in communication_statistic, api_statistic,"
    ...
    def view_summary_content(self, summary_dir):
        for index, csv in enumerate(sorted(os.listdir(summary_dir))):
            ...
            if "ai_core_utilization" == csv_name or "op_summary" == csv_name or "ai_vector_core_utilization" == csv_name:
                ...
                if not re.findall(r"aic-metrics\S+", str(self.switch_values)):
                    if "AclApi" == self.__scene or "AclJson" == self.__scene or "ExportTensorFlow" == self.__scene \
                        ...
                        aic_mode = "task-based"
                    elif "nano_pipeutilization" == self.__scene:
                        aic_metrics = "NANO_PipeUtilization"
                        aic_mode = "task-based"
                    elif "nano_pipeStallCycle" == self.__scene:
                        aic_metrics = "NANO_PipeStallCycle"
                        aic_mode = "task-based"
                    elif "nano_memory" == self.__scene:
                        aic_metrics = "NANO_Memory"
                        aic_mode = "task-based"
                    elif "nano_memoryUB" == self.__scene:
                        aic_metrics = "NANO_MemoryUB"
                        aic_mode = "task-based"
                    elif "nano_scalarRatio" == self.__scene:
                        aic_metrics = "NANO_ScalarRatio"
                        aic_mode = "task-based"
                    else:
                        aic_metrics = "PipeUtilization"
                    ...
                    if "AddOp" == self.__scene or "MsprofbinPyTorch" == self.__scene \
                        ...
                        if "ai_core_utilization" == csv_name or "ai_vector_core_utilization" == csv_name:
                ...
                if "communication_statistic" == csv_name:
    ...
    def compare_output_file(self, mindstudio_profiler_output, switch_res):
        """
        ...
        for index, file in enumerate(all_files):
            ...
            if "slice" in file_name:
                file_name = file_name.rstrip("_slice")
            if "cache" == file_name:
                file_name = "l2_cache"
        ...
        expected_timeline_files = set(
            switch_res.get("dev_json", []) + switch_res.get("host_json", [])) & all_timeline_files
    ...
    def view_timeline_content(self, mindstudio_profiler_output):
        ...
        检查msprof.json文件的总大小
        """
        msprof_json_path = os.path.join(mindstudio_profiler_output, "msprof*.json")
        if not os.path.exists(msprof_json_path):
            self.__logger.info("timeline msprof*.json is lost.")
            return
        with open(msprof_json_path, 'r', encoding='utf-8') as txtfile:
    ...
    def view_res_file(self, output, switch_res):
        if "AclApiSubscription" == self.__scene:
            ...
            dev_dir_list, log_dir_list, output_dir_list = self._get_dir_list(output)
            try:
                ...
                for output_dir in output_dir_list:
                    self.compare_output_file(output_dir, switch_res)
                    self.view_summary_content(output_dir)  # 检查csv文件
                    self.view_timeline_content(output_dir)
                    json_check = MsprofJsonChecker(output_dir, self.start_timestamp_us,
                                                   self.end_timestamp_us, self.__id, "910B")
    ...
    def view_plog_and_res_file(self, switch_res):
    ...
    def case_single_switch(self, params=None, scene=None):
        ...
        self.view_plog_and_res_file(switch_res)
    ...
    def case_app_end_switch(self, params=None, scene=None):
        ...
        if "single_switch" == scene:
            msprofbin_cmd = "cd {}/out/; {} --output={} {} > {}".format(self.cfg_path.infer_path,
                                                                        self.cfg_cmd.single_app_end_switch,
                                                                        self.res_dir,
    ...
    def case_custom(self, params=None):
        ...
        msprofbin_cmd = self.cfg_cmd.single_app_end_switch + \
                        " --ascendcl=on --model-execution=on --task-time=on --ai-core=on --aic-mode=task-based "
        if params is not None:
            params = params.replace(",", " ")
            msprofbin_cmd = msprofbin_cmd + params + f"--output={self.res_dir} -> {self.slog_stdout}"
        self.run_cmd(msprofbin_cmd)
        if self.__id == "test_aclrtSetDevice_more_than_once":
            self.check_test_aclrtSetDevice_more_than_once(msprofbin_cmd)
        else:
            switch_res = self.get_switch_res(msprofbin_cmd)
            self.view_plog_and_res_file(switch_res)
    ...
    def case_api_scene(self, scene=None):
        ...
        self.run_cmd(cmd)
        switch_res = self.get_switch_res(cmd)
        self.view_plog_and_res_file(switch_res)
    ...
    def case_train_scene(self, scene=None):
        ...
        elif "ImageNetTrain" == scene:
            train_path = self.cfg_path.ImageNetTrain
        elif "MS-llama2" == scene:
            train_path = self.cfg_path.MindSporeLlama2
        elif "Pytorch-llama2-singleOp" == scene:
            train_path = self.cfg_path.PytorchLlama2Single
        elif "Pytorch-llama2-graph" == scene:
            train_path = self.cfg_path.PytorchLlama2Graph
        else:
            train_path = ""
    ...
    def get_analysis_switch_res(self, cmd, scene):
        dev_json_lst = []
        dev_csv_lst = []
        host_json_lst = []
        host_csv_lst = []
        switch_res_dict = {}
        if "nano_pipeutilization" == scene or \
                "nano_pipeStallCycle" == scene or \
                "nano_memory" == scene or \
                "nano_memoryUB" == scene or \
                "nano_scalarRatio" == scene:
            dev_json_lst.extend(['msprof', 'task_time'])
            dev_csv_lst.extend(['op_statistic', 'op_summary', 'task_time'])
        elif "Helper" == scene:
            ...
            if "ffts_off_hccl_l1" == scene or "ffts_on_hccl_l1" == scene:
                dev_csv_lst.extend(['op_statistic', 'communication_statistic'])
    ...
    def case_analysis_scene(self, scene=None):
        ...
        analysis_paths = {
            ...
            "nano_pipeutilization": self.cfg_path.nanoPipeUtilization_path,
            "nano_memory": self.cfg_path.nanoMemory_path,
            "nano_memoryUB": self.cfg_path.nanoMemoryUB_path,
            "nano_pipeStallCycle": self.cfg_path.nanoPipeStallCycle_path,
            "nano_scalarRatio": self.cfg_path.nanoScalarRatio_path,
        ...
        cmd = "cd {}; {}; bash run_analysis.sh {} > {}".format(analysis_path, self.cfg_path.toolkit_env_path,
                                                               self.res_dir, self.slog_stdout)
        self.run_cmd(cmd)
        switch_res = self.get_analysis_switch_res(cmd, scene)
        self.view_plog_and_res_file(switch_res)
    ...
    def case_pytorch_profiling(self, scene=None):
        ...
        if "no_profiling" == scene:
            ...
            cmd = "cd {}; {}; bash run_lenet_framework_profiling.sh {} > {}".format(app_path,
    ...
    def case_parse_performance(self, scene=None):
        ...
        t1 = time.time()
        self.run_cmd(msprofbin_cmd)
        switch_res = self.get_analysis_switch_res(msprofbin_cmd, scene)
        self.view_plog_and_res_file(switch_res)
        run_t = time.time() - t1
    ...
    def check_test_aclrtSetDevice_more_than_once(self, msprofbin_cmd):
        switch_res = self.get_switch_res(msprofbin_cmd)
        dev_dir_list, log_dir_list, output_dir_list = self._get_dir_list(self.res_dir)
        for output_dir in output_dir_list:
            self.compare_output_file(output_dir, switch_res)
            self.view_timeline_content(output_dir)
    ...
    def _get_dir_list(self, result_dir):
        """
        获取result_dir下面的所有PROF目录的device_*, mindstudio_profiler_log和mindstudio_profiler_output
        result_dir是存放结果文件的目录，对应/home/result_dir下面的各个用例
        """
        prof_lst = []
        for prof_ in os.listdir(result_dir):
            if os.path.isfile(os.path.join(result_dir, prof_)):
                continue
            prof_lst.append(os.path.join(result_dir, prof_))
        return self.get_dev_log_output_list(prof_lst)
