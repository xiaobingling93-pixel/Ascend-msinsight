...
function Main()
{
    ...
    ./auto_ssh.sh "cd /home/msprof_smoke_test/smoke_test/910b_arm/testcase; bash run_all.sh ${command_type} ${msprof_version} ${parsePerformance}" ${host_ip} ${host_user} ${host_pwd} | tee -a ${run_log}
    CopyLogPROFData
    sshpass -p ${host_pwd} scp -r -q -o StrictHostKeyChecking=no ${host_user}@${host_ip}:/home/msprof_smoke_test/smoke_test/910b_arm/testcase/result.txt ./
