...
rm -rf $PROJECT_PATH/output/multimodal_sd_modelslim_v1_hunyuan_video_w8a8_mxfp8
mkdir $PROJECT_PATH/output/multimodal_sd_modelslim_v1_hunyuan_video_w8a8_mxfp8
...
msmodelslim quant \
    --model_path $PROJECT_PATH/resource/models/HunyuanVideo \
    --save_path $PROJECT_PATH/output/multimodal_sd_modelslim_v1_hunyuan_video_w8a8_mxfp8 \
    --device npu \
    --model_type hunyuan_video \
    --config_path $PROJECT_PATH/test-case/multimodal_sd_modelslim_v1_hunyuan_video_w8a8_mxfp8/hunyuan_video_w8a8_mxfp8.yaml \
    --trust_remote_code True

# 配置待检查的路径和文件列表
TARGET_PATH="$PROJECT_PATH/output/multimodal_sd_modelslim_v1_hunyuan_video_w8a8_mxfp8"  # 修改为实际路径
FILES=(
...
# 检查 quant_model_weight_w8a8_mxfp8.safetensors 或其分片文件
quant_model_single="$TARGET_PATH/quant_model_weight_w8a8_mxfp8.safetensors"
quant_model_pattern="$TARGET_PATH/quant_model_weight_w8a8_mxfp8-*.safetensors"
quant_model_files=($quant_model_pattern)

if [ ! -e "$quant_model_single" ] && [ ${#quant_model_files[@]} -eq 0 ]; then
    echo "错误：quant_model_weight_w8a8_mxfp8.safetensors 或其分片文件不存在" >&2
    all_exist=false
else
