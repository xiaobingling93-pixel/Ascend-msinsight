...
# 用例名（用于打屏和指定输出路径）
CASE_NAME=modelslim_v1_qwen3_dense_w16a16s
...
msmodelslim quant \
    --model_path $PROJECT_PATH/resource/llm_ptq/Qwen3-14B-2layers \
    --save_path $PROJECT_PATH/output/$CASE_NAME \
    --device npu \
    --model_type Qwen3-14B \
    --config_path $PROJECT_PATH/test-case/$CASE_NAME/dense-w16a16s-v1.yaml \
