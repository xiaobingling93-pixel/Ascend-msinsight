...
CONFIG_SCRIPT_NAME="mindie_infer_triton_stream.py"
...
# 添加外部传入的信息给模型配置文件(具体模型配置文件路径请依据实际情况修改, 模型或数据集配置文件统一命名成CASE_NAME)
{
    ...
    echo "models[0]['model_name'] = '${DEFAULT_MODEL_NAME}'"
...
req_metric_list=("E2EL" "TTFT" "TPOT" "ITL" "LastITL" "MaxITL" "InputTokens" "OutputTokens" \
    "OutputTokenThroughput" "GeneratedCharacters" "PrefillBatchsize" "DecoderBatchsize" "QueueWaitTime")
